...
def make_dirl_goal_map(n_maps) :
    ...
    for i in range(n_maps):
        ...
        plt.scatter(4.5, 6.5, color='white', marker='*', s=100)
        plt.scatter(6.5, 4.5, color='red', marker='*', s=100)
        # 제목 추가
        if i == 0:
             ...
             plt.title('Free',fontweight="bold",fontsize=20)
        # 그림 저장 (DPI 설정 추가)
...
address = 'irl/'+monkey+'/day2/3_9_map4_time9_bias[25, 25, 25, 0]'
