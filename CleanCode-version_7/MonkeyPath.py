...
class MonkeyPath:
    ...
    def get_sa(self, day, start_trial, size, T) :
        ...
        for trial in trials :
            ...
            if len(sa) < T :
                for j in range(len(sa)+1,T+1) :
                    sa.append([self.to_index[tuple(trial[-1])], 4])
                trajectories.append(sa)
            else :    
                trajectories.append(sa)

        with open('parameters/trajectories.pickle', 
                  'wb') as f:
...
if __name__ == "__main__":
    ...
    print(monkey_path.get_goal_map(2,10,3)[2])
    print(len(monkey_path.new_pos[4]))
# %%
