...
from agent_package.agent_model_based import Agent 
from agent_package.CuriosityExploreModelTable import CuriosityExploreModelTable
...
def pre_train(agent = Agent(1,1,1,1,1,1,1), model = CuriosityExploreModelTable, episode_num = 10): 
    max_episode = 100 
...
game_name = "curiosity_explore_model"
...
task_info = f"Task at time: {str(run_time)} is curiosity explore model based training with pre_training = {str(bool_pre_train)} and per {str(bool_PER)}\n"
...
model = CuriosityExploreModelTable(memory_size=10, data_dir = data_dir)
